...
define(function(require, exports) {
  ...
  class Timeline {
    ...
    init() {
      ...
      // dayAxis Scale
      this.xDayScale = this.d3.scaleLinear().range([0, this.width])
      // dateAxis Scale
      this.xDateScale = this.d3.scaleTime().range([0, this.width])
      // create brush
    ...
    updateData(rawData) {
      ...
      this.originalData = this.transformedData(rawData).filter(
    ...
    drawTimeline(chartData) {
      ...
      timelineChildren
        .selectAll('line')
        .data(d =>
          d.observationData.filter(el => el.endMoment !== el.startMoment)
        )
        .enter()
        .append('line')
      ...
      circles
        ...
        .append('circle')
        .attr('cx', d => {
          return this.xDayScale(d.startMoment)
        })
        .attr(
          'style',
          d =>
    ...
    showTooltip(timeLineData, d) {
      ...
      tooltip
        .style('left', `${this.d3.event.pageX - tooltipSize.width / 2 - 174}px`)
      ...
      timelineObservationData
        ...
        .forEach(point => {
          const pointIndex = tooltipContentList.findIndex(
            p =>
              p.startMoment === point.startMoment &&
    ...
    remove() {
      ...
      this.d3.select('.profileTimeline').remove()
    }
  }
