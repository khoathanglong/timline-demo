...
function main(htmlElement, myData, d3, _) {
  ...
  const pinFill = 'white';
  const expandAllColor = '#467AB2';
  // filter input
  const filterInput = d3.select(htmlElement)
    .append('div')
    .attr('class', 'timelineFilter')
    .style('text-align', 'left')
    .append('input')
    .attr('placeholder', 'Filter timeline');
  d3.select('.timelineFilter')
    .style('padding', `${margin.top / 2}px 0 0 ${margin.left}px`);


  const expandAll = d3.select(htmlElement)
    .append('div')
    .text('Expand all')
    .attr('class', 'timelineFilter')
    .style('text-align', 'left')
    .style('color', expandAllColor)
    .style('padding', `${margin.top / 3}px 0 0 ${margin.left}px`);
  ...
  expandAll.on('click', () => updateData(detailedData));
  ...
  function updateData(filteredData) {
    ...
    const width = document.getElementById(htmlElement).offsetWidth - margin.left - margin.right;
